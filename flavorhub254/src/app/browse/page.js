    ...
    useEffect(() => {
        function handleResize() {
            if (window.innerWidth >= 640) { // Tailwind's sm breakpoint is 640px
                setRecipesPerPage(9); // 3x3 on desktop
            } else {
                setRecipesPerPage(4); // 2x2 on mobile
            }
        }
        handleResize();
...
export default function BrowsePage() {
    ...
    const [recipePage, setRecipePage] = useState(0); // <-- move this up!
                    ...
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/40 to-black/80">
                        ...
                        <div className="flex items-center justify-between px-4 sm:px-15 pt-3 sm:pt-5 bg-black/60 sm:bg-black/12 rounded-b-xl ">
                            ...
                            <span className="text-2xl sm:text-3xl font-bold leading-none">
                                Browse
                                <span style={{ color: "#D32F2F" }}>Reci</span>
                                <span style={{ color: "#2E7D32" }}>pes</span>
                            </span>
                            </Link>
